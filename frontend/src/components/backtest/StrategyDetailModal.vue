<template>
  <div v-if="show" class="fixed inset-0 bg-black/50 flex items-center justify-center z-50">
    <div class="bg-gray-800 p-6 rounded-xl w-[800px] max-h-[90vh] overflow-y-auto border border-gray-700">
      <!-- 헤더 -->
      ...
      <div class="mb-6">
        ...
        <div class="grid grid-cols-2 gap-4">
          <div v-for="(param, key) in strategy.parameters" :key="key" class="bg-gray-700/50 p-3 rounded-lg">
      ...
      <div class="mb-6">
        ...
        <div class="bg-gray-900 p-4 rounded-lg">
          <img :src="strategy.chartExample" :alt="strategy.name + ' 차트 예시'" class="w-full rounded-lg" />
        </div>
      </div>
...
const emit = defineEmits(['close', 'try-strategy'])
...
const strategy = computed(() => {
  const strategies = {
    ma_crossover: {
      name: '이동평균선 크로스오버',
      description: '단기 이동평균선이 장기 이동평균선을 상향 돌파할 때 매수하고, 하향 돌파할 때 매도하는 전략입니다. 추세 추종형 전략으로, 강한 상승/하락 추세에서 효과적입니다.',
      parameters: [
        {
          name: '단기 이동평균선 기간',
      ...
      chartExample: '/images/strategies/ma_crossover.png',
      buySignal: '단기 이동평균선이 장기 이동평균선을 상향 돌파할 때',
      sellSignal: '단기 이동평균선이 장기 이동평균선을 하향 돌파할 때'
    },
    rsi: {
      name: 'RSI 과매수/과매도',
      description: 'RSI(상대강도지수)가 과매도 구간에서 반등할 때 매수하고, 과매수 구간에서 하락할 때 매도하는 전략입니다. 반등 매매에 적합한 전략입니다.',
      parameters: [
        {
          name: 'RSI 기간',
      ...
      chartExample: '/images/strategies/rsi.png',
      buySignal: 'RSI가 과매도 구간(30 이하)에서 반등할 때',
      sellSignal: 'RSI가 과매수 구간(70 이상)에서 하락할 때'
    },
    bollinger: {
      name: '볼린저 밴드 반전',
      description: '가격이 볼린저 밴드의 하단을 터치하면 매수하고, 상단을 터치하면 매도하는 전략입니다. 변동성 돌파 전략으로 적합합니다.',
      parameters: [
        {
          name: '기간',
    ...
    macd: {
      name: 'MACD 크로스오버',
      description: 'MACD 라인이 시그널 라인을 상향 돌파할 때 매수하고, 하향 돌파할 때 매도하는 전략입니다. 추세 전환을 포착하는데 효과적입니다.',
      parameters: [
        {
          name: '빠른 이동평균선 기간',
      ...
      chartExample: '/images/strategies/macd.png',
...
const tryStrategy = () => {
  emit('try-strategy', props.strategyId)
  emit('close')
}
</script> 